...
def app():
    ...
    st.write("This page displays the metrics and visualizations from the dataset")
    ...
    def barChart(data, title, X, Y):
        ...
        msgChart = (
            alt.Chart(data).mark_bar().encode(alt.X(f"{X}:N", sort=alt.EncodingSortField(field=f"{Y}", op="values",
                                                                                         order='ascending')),
                                              y=f"{Y}:Q"))
    ...
    selectHashTag()
    st.markdown(
        "<p style='padding:10px; background-color:#000000;color:#00ECB9;font-size:16px;border-radius:10px;'>Section Break</p>",
